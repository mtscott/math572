...
#Ad =-(beta/h)**2*dt/2*sp.diags([1., -2., 1.], [-1, 0, 1], shape=[Nx+1, Nx+1], format = 'csr')
if beta>0:
    Ac =beta/(h)*sp.diags([-1., 1.], [-1, 0], shape=[Nx+1, Nx+1], format = 'csr')
else:
    Ac =beta/(h)*sp.diags([1, -1.], [1, 0], shape=[Nx+1, Nx+1], format = 'csr')
...
RightM = sp.identity(Nx+1, format = 'csr') - dt*A
