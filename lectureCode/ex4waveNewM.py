"""
MATH572 Linear Advection Problems with Upwind
...
A =sp.diags([1., -2., 1.], [-1, 0, 1], shape=[Nx+1, Nx+1], format = 'csr')
#Ar = sigma*sp.identity(Nx+1, format = 'csr')

LeftM = sp.identity(Nx+1, format = 'csr') - cfl2/4*A
RightM0 = 2*sp.identity(Nx+1, format = 'csr') + cfl2/2*A
RightM1 = -sp.identity(Nx+1, format = 'csr') + cfl2/4*A
...
#     LeftM*u_n = RightM0*u_nm1 + RightM1*(u_n - 2*dt*ICP(x))

b = RightM0*u_nm1 - 2*dt*RightM1*ICP(x)
LeftMIC = LeftM - RightM1
b[0], b[-1] = BC(xl, xr,tc)
LeftMIC[0,0] = 1.
LeftMIC[0,1] = 0.
LeftMIC[-1,-1] = 1.
LeftMIC[-1,-2] = 0.
print(LeftMIC)
u_n = sp.linalg.spsolve(LeftMIC, b)    
visualize(x,tc,u_n)
